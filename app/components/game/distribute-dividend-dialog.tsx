...
export function DistributeDividendDialog({
  companyId,
  companyName,
  companyBalance,
  companyOwnerId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (dividendAmount > companyBalance) {
      toast({
        ...
        description: `Company only has $${companyBalance.toFixed(2)} available`,
        variant: "destructive",
      });
      return;
    }
            ...
            {investorCount === 0 && (
              <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4">
                <p className="text-sm text-yellow-600">
                  Warning: No external investors found. You need investors to distribute dividends.
                </p>
              </div>
            )}
          </div>
